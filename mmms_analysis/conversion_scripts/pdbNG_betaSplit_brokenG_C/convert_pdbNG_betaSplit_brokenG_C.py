...
if __name__ == "__main__":
    ...
    # cut merger posterior samples by a np.where on a single column "sig_peak1_NS" inside merged_posterior_samples
    sig_peak_NS_median = np.median(merged_posterior_samples["sig_peak_NS"])
    mask = merged_posterior_samples["sig_peak_NS"] < sig_peak_NS_median
    merged_posterior_samples = {key: val[mask].reshape(-1, 1) for key, val in merged_posterior_samples.items()}
    ...
    num_samples = len(merged_posterior_samples["alpha_1"])
