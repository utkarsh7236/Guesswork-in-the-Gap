...
if __name__ == "__main__":
    ...
    gamma_high_min = float(np.min(merged_posterior_samples["gamma_high"]))
    gamma_high_max = float(np.max(merged_posterior_samples["gamma_high"]))
    eta_high_min = float(np.min(merged_posterior_samples["eta_high"]))
    eta_high_max = float(np.max(merged_posterior_samples["eta_high"]))
    np.random.seed(7236)
    gamma_high = np.random.uniform(low = gamma_high_min, high = gamma_high_max, size=converted_posterior_samples["notch_lowmass_scale"].shape[0])
    eta_high = np.random.uniform(low = eta_high_min, high = eta_high_max, size=converted_posterior_samples["notch_lowmass_scale"].shape[0])
    converted_posterior_samples["notch_highmass_scale"] = gamma_high
    converted_posterior_samples["notch_highmass_exponent"] = eta_high

    for key, value in converted_posterior_samples.items():
        if key not in ["notch_highmass_scale", "notch_highmass_exponent"]:
