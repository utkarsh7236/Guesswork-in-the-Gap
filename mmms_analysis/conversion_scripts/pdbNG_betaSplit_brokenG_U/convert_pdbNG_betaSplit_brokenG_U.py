...
if __name__ == "__main__":
    ...
    merged_posterior_samples["mix_tilt1"] = replace("mix_tilt1", 1)
    merged_posterior_samples["mix_tilt2"] = replace("mix_tilt2", 1)
    ...
    for new_key, old_key in conversion_dict.items():
        ...
        if new_key in gaussian_mixture_list:  # for items in the gaussian mixture list.
            ...
            mu_costilt = 0
            converted_posterior_samples[new_key] = mu_costilt*np.ones((merged_posterior_samples["alpha_1"].shape[0], 1))
        else:
