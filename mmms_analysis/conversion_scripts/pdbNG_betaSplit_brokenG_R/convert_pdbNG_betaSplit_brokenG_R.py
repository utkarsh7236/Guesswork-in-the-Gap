...
if __name__ == "__main__":
    ...
    beta_1_min = float(np.min(merged_posterior_samples["beta_1"]))
    beta_1_max = float(np.max(merged_posterior_samples["beta_1"]))
    beta_2_min = float(np.min(merged_posterior_samples["beta_2"]))
    beta_2_max = float(np.max(merged_posterior_samples["beta_2"]))
    np.random.seed(7236)
    beta_1 = np.random.uniform(low = beta_1_min, high = beta_1_max, size=converted_posterior_samples["pairing_pow_q_low"].shape[0])
    beta_2 = np.random.uniform(low = beta_2_min, high = beta_2_max, size=converted_posterior_samples["pairing_pow_q_low"].shape[0])
    converted_posterior_samples["pairing_pow_q_low"] = beta_1
    converted_posterior_samples["pairing_pow_q_high"] = beta_2

    # assert that in converted_posterior_samples everything is constant except pairing_pow_q_low and pairing_pow_q_high
    for key, value in converted_posterior_samples.items():
        if key not in ["pairing_pow_q_low", "pairing_pow_q_high"]:
