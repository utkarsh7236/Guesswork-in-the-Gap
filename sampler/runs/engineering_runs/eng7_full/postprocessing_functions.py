...
def plot_p_costilt(posterior_samples, function  = None):
    ...
    try:
        ...
        m_above = 10
        msb = 5
        p_costilt_below = function(costilt, **params_samples, m = m_below, m_spin_break=msb)
        p_costilt_above = function(costilt, **params_samples, m = m_above, m_spin_break=msb)
        ...
        plt.fill_between(costilt, p05_below, p95_below, color="dodgerblue", alpha=0.2, label="90\% CI")
        plt.plot(costilt, p50_below, color="dodgerblue", label = f"Below {msb} M$_\odot$")
        plt.fill_between(costilt, p05_above, p95_above, color="darkblue", alpha=0.2)
        plt.plot(costilt, p50_above, color="darkblue", label = f"Above {msb} M$_\odot$")
